...
export function SitesPage({ sites, onSiteCreated }: SitesPageProps) {
  ...
  const filteredSites = sites.filter((site) =>
    site.site_domain.toLowerCase().includes(searchQuery.toLowerCase()),
  )
      ...
      <div className="bg-[#161619] border border-[#232328] rounded-lg overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-[#232328] flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between bg-[#161619]">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <button className="flex items-center gap-1.5 px-3 py-2 bg-[#202024] border border-[#2d2d34] rounded hover:bg-[#2c2c32] transition font-semibold text-white">
              Filters
              <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />
            </button>

            <div className="flex items-center bg-[#121214] border border-[#2d2d34] rounded">
              <button className="flex items-center gap-1 px-2.5 py-2 border-r border-[#2d2d34] text-muted-foreground hover:text-white transition font-semibold">
                Search By
              ...
              <input
                ...
                value={searchQuery}
              />
              <button className="p-2 text-muted-foreground hover:text-white transition">
                <Search className="h-3.5 w-3.5" />
              </button>
            </div>

            <button className="p-2 bg-[#202024] border border-[#2d2d34] rounded hover:bg-[#2c2c32] text-muted-foreground hover:text-white transition">
              ...
              {filteredSites.length === 0 ? (
                ...
                filteredSites.map((site) => (
                  <tr className="hover:bg-[#1c1c20] transition" key={site.id}>
                    ...
                    <td className="px-5 py-4">
                      {/* Cloudflare cloud badge */}
                      <Cloud className="h-5 w-5 text-[#88888b] fill-[#88888b]/10" />
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-3">
                        <Link
                          className="px-3 py-1.5 bg-[#202024] hover:bg-[#2c2c32] rounded border border-[#2d2d34] text-[11px] font-semibold transition"
                          to={`/sites/${site.id}`}
                        >
                          Environments
                        </Link>
