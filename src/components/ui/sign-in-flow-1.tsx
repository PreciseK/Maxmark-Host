...
const ShaderMaterial = ({
  ...
  // Shader material — uniforms are prepared inside the memo so the
  // dependency list is honest (uniforms, size, source).
  ...
  }, [uniforms, size.width, size.height, source]);
...
function MiniNavbar({
  ...
  const logoElement = (
    <a className="flex items-center gap-2 font-mono text-[14.5px] font-bold text-white tracking-tight lowercase select-none no-underline hover:text-white/80 transition-colors" href="/index.html">
      <span className="w-[7px] h-[7px] rounded-full bg-[#5c4df0]" />
      maxmark
    </a>
  );
  ...
  const loginButtonElement = (
    <button
      ...
      className={cn(
        "px-4 py-2 sm:px-3 text-xs sm:text-sm rounded-full transition-all duration-200 w-full sm:w-auto border",
        !isSignUp
          ? "border-[#5c4df0] bg-[#5c4df0]/20 text-white font-semibold"
  ...
  const signupButtonElement = (
    <div className="relative group w-full sm:w-auto">
       {isSignUp && (
         <div className="absolute inset-0 -m-1.5 rounded-full hidden sm:block bg-[#5c4df0] opacity-35 filter blur-md pointer-events-none transition-all duration-300 ease-out group-hover:opacity-50 group-hover:blur-lg" />
       )}
       <button
         onClick={onSignupClick}
         className={cn(
           "relative z-10 px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold rounded-full transition-all duration-200 w-full sm:w-auto border",
           isSignUp
             ? "text-white bg-[#5c4df0] hover:bg-[#4d3fe0] border-transparent"
             : "text-black bg-gradient-to-br from-gray-100 to-gray-300 hover:from-gray-200 hover:to-gray-400 border-transparent"
         )}
       >
         Signup
       </button>
...
export const SignInPage = ({ className }: SignInPageProps) => {
  ...
  // Demo mode (no Supabase credentials) keeps the flow walkable; live mode
  // sends a real email OTP and only advances on a verified code.
  ...
  const handleCodeChange = (index: number, value: string) => {
    if (value.length <= 1) {
      ...
      // Verify once the last digit is entered
      if (index === 5 && value) {
                ...
                {step === "email" ? (
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <p className="text-[1.3rem] text-white/70 font-light">
                        {isSignUp ? "Get started with Maxmark Host" : "Sign in to your Maxmark account"}
                      </p>
                    </div>
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.25rem] text-white/50 font-light">Please enter it to verify your email</p>
                    </div>
                    ...
                    <div className="flex w-full gap-3">
                      ...
                      >
                        {busy ? "Verifying..." : isSignUp ? "Sign Up" : "Sign In"}
                      </motion.button>
                    </div>
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[1.25rem] text-white/50 font-light">
                        {isSignUp ? "Welcome to Maxmark Host" : "Welcome back"}
                      </p>
                    </div>
