-- ─── Clean stale reservations and refine capacity reservation check ────────
...
begin
  ...
  if plan_limit <= 0 then
    raise exception using errcode = 'P0001', message = 'An active hosting plan is required. Please activate or upgrade your plan.';
  end if;
  ...
  if exists (select 1 from public.user_sites where site_domain = target_site_domain and status <> 'failed') then
    raise exception using errcode = 'P0001', message = 'Domain "' || target_site_domain || '" is already provisioned on Maxmark Host. Please enter a different domain name.';
  end if;
...
-- Clean out any existing stale reservations
delete from public.site_provisioning_reservations;
